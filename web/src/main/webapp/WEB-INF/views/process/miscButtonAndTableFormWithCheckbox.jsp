...
window.onload=function(){
	...
	showAlreadyProcessedMsgAfterCheckedDataPost();//show msg if other user already processed book
}
...
function showAlreadyProcessedMsgAfterCheckedDataPost(){
	
	var sel= document.getElementById("tnAlreadyProcessedData");
	var alreadyProcessedData = sel.value;
 
	if(alreadyProcessedData != ""){
		alert("The following TN(s) was already processed by another user: \n" + alreadyProcessedData + "\n");
	} 
}
  ...
  <div class="row-fluid">
    <div class="span"> 
		...
		<sf:form id="f1" class="" name="f1" method="post" action="${buttonsAction}" modelAttribute="book">
			...
			<input type="hidden" id="tnAlreadyProcessedData" value="${infoMsg}"/>
			...
			<tr>
				...
				<c:forEach var="b" items="${buttons}">
					...
					<c:when test='${b.get(0).equals("exportButton") == true}'>
						<td><button id="${b.get(0)}" name="exportButton" value="${b.get(0)}" onclick="tableToExcel('tnTable', 'book list'); return false;">${b.get(1)}</button></td>
					</c:when>
					<c:otherwise>
			 ...
			 <table id="tnTable" class="sortable colSpace">
				...
				<tr>
					...
					<td valign="top" align="left">
						...
						<a  href="trackingForm?update&tn=${tn.get(tnColumnNumber)}&returnTo=${returnTo}" ><c:out value="${tn.get(i)}"/>
					</td>
