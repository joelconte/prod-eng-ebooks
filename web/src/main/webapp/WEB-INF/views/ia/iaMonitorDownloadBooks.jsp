...
function showDetailsOverlay(selectedRow, selected, bibCheck, identifier, title, imageCount, language, publishDate, subject, description, publisher, licenseUrl, rights, author, oclc, tn, dnp, error){
	...
	document.getElementById('ol_error').value=error;
				...
				<tr>
					...
					<c:forEach var="i" begin="0" end="${colLabels.size()-1}">
						<td id="cell${rowNum}_${i}" valign="top" align="left">
							...
							<c:when test='${i==6}'>
								<c:choose>
								<c:when test="${ fn:length( fieldValue ) le 9999}" > 
									<a title="View Details" href="javascript:void(0);" onclick="showDetailsOverlay( '${rowNum}', '${row.get(4)}', '${row.get(5)}', '${row.get(6)}', '${row.get(7)}','${row.get(8)}', '${row.get(9)}', '${row.get(10)}', '${row.get(11)}', '${row.get(12)}', '${row.get(13)}', '${row.get(14)}', '${row.get(15)}', '${row.get(16)}', '${row.get(17)}', '${row.get(18)}', '${row.get(19)}', '${row.get(22)}'); return false;" ><c:out value="${fieldValue}"/></a><!-- 35 old max len. Show all of identifier since Map of ids needs to ensure unique values in map-->
								</c:when>
								<c:otherwise>
									<a title="View Details" href="javascript:void(0);" onclick="showDetailsOverlay( '${rowNum}', '${row.get(4)}', '${row.get(5)}', '${row.get(6)}', '${row.get(7)}','${row.get(8)}', '${row.get(9)}', '${row.get(10)}', '${row.get(11)}', '${row.get(12)}', '${row.get(13)}', '${row.get(14)}', '${row.get(15)}', '${row.get(16)}', '${row.get(17)}', '${row.get(18)}', '${row.get(19)}', '${row.get(22)}'); return false;" ><c:out value="${fn:substring(fieldValue, 0, 34)}..."/></a>
								</c:otherwise>
								</c:choose>
							</c:when>
							<c:otherwise>
