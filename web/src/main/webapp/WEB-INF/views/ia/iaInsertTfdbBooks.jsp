...
function showDetailsOverlay(selectedRow, selected, bibCheck, identifier, title, imageCount, language, publishDate, subject, description, publisher, licenseUrl, rights, author, oclc, tn, dnp){
	...
	var selected = document.getElementById('cell' + selectedRow + '_4').innerHTML;
	if(selected.trim() == 'T'){
	...
	var oclc = document.getElementById('cell' + selectedRow + '_17').innerHTML.trim();
	var tn = document.getElementById('cell' + selectedRow + '_18').innerHTML.trim();

	var dnp = document.getElementById('cell' + selectedRow + '_19').innerHTML;
	if(dnp.trim() == 'T'){
...
function doBooksToInsert(firstShowOverlay){
  ...
  <div class="row-fluid">
    <div class="span"> 
		...
		<form id="f1" class="" name="f1" method="post" >
	 	   ...
	 	   <td><button id="release" name="button" value="release"  onclick="doBooksToInsert(true); return false; ">DONE - Insert into FamilySearch</button></td>
	    	</tr>
			</table>
						...
						<td id="cell${rowNum}_${i}" valign="top" align="left">
							...
							<c:when test='${i==6}'>
								...
								<c:when test="${ fn:length( fieldValue ) le 35}" >
									<a title="View Details" href="javascript:void(0);" onclick="showDetailsOverlay( '${rowNum}', '${row.get(4)}', '${row.get(5)}', '${row.get(6)}', '${row.get(7)}', '${row.get(8)}', '${row.get(9)}', '${row.get(10)}', '${row.get(11)}','${row.get(12)}', '${row.get(13)}', '${row.get(14)}', '${row.get(15)}', '${row.get(16)}', '${row.get(17)}',  '${row.get(18)}','${row.get(19)}'); return false;" ><c:out value="${fieldValue}"/></a>
								</c:when>
								<c:otherwise>
									<a title="View Details" href="javascript:void(0);" onclick="showDetailsOverlay( '${rowNum}', '${row.get(4)}', '${row.get(5)}', '${row.get(6)}', '${row.get(7)}', '${row.get(8)}', '${row.get(9)}', '${row.get(10)}', '${row.get(11)}','${row.get(12)}', '${row.get(13)}', '${row.get(14)}', '${row.get(15)}', '${row.get(16)}', '${row.get(17)}',  '${row.get(18)}', '${row.get(19)}'); return false;" ><c:out value="${fn:substring(fieldValue, 0, 34)}..."/></a>
								</c:otherwise>
								</c:choose>
							</c:when>
							<c:otherwise>
...
<div id="specialBox2"  style="position:fixed; width:450px; height: 150px; left:20px; top:20px;  display: none;">
  ...
  <form id="olf2" name="olf2" action="" method="post">
    PDF Orem Drive Name
    <input type="text" id="ol_driveName"/>
    <br>
    PDF Orem Drive Serial #
    <input type="text" id="ol_driveNumber"/>
    
	<br>   <br>
  	<button onclick="closeOverlay2( true  ); return false;" id="btnSave" name="button" value="closeSave" >Save Selections</button>	
	<button onclick="closeOverlay2( false ); return false;" id="btnCancel" name="button" value="cancel" >Cancel</button>				
  </form>
</div>
